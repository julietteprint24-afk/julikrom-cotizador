...
type Complejidad = "basica" | "media" | "avanzada";
...
function FigurasPVC({ volverMenu }: Props) {
  const [descripcion, setDescripcion] = useState("Figura PVC personalizada");
  const [costoProveedor, setCostoProveedor] = useState(10);
  const [cantidad, setCantidad] = useState(1);
  const [complejidad, setComplejidad] = useState<Complejidad>("basica");
  ...
  const costoComplejidad =
    costoProveedorTotal * (multiplicadores[complejidad] - 1);
  ...
  return (
    <div className="calculadora">
      ...
      <p className="subtitulo">
        Calcula piezas, letras, figuras y formas PVC usando precio proveedor.
      </p>
      ...
      <div className="selector-opciones">
        <button
          className={complejidad === "basica" ? "opcion activa" : "opcion"}
          onClick={() => setComplejidad("basica")}
        >
          Básica
        </button>

        <button
          className={complejidad === "media" ? "opcion activa" : "opcion"}
          onClick={() => setComplejidad("media")}
        >
          Media
        </button>

        <button
          className={complejidad === "avanzada" ? "opcion activa" : "opcion"}
          onClick={() => setComplejidad("avanzada")}
        >
          Avanzada
        </button>
      </div>
      ...
      <p className="nota">
        Básica: pieza simple. Media: más detalles. Avanzada: letras, siluetas o
        formas más personalizadas.
      </p>
      ...
      <div className="resultado">
        ...
        <div className="precio-card">
          ...
          <strong>${costoComplejidad.toFixed(2)}</strong>
        </div>
      ...
      <div className="consejo">
        ...
        <p>
          Usa el precio real del proveedor y ajusta la complejidad según el
          nivel de detalle de la pieza.
        </p>
      </div>
    </div>
  );
